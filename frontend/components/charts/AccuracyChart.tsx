...
export function AccuracyChart({ combos }: Props) {
  ...
  const data = classifiers.map((clf) => {
    ...
    for (const dr of drMethods) {
      ...
      row[dr] = match ? Number((match.test_accuracy * 100).toFixed(1)) : 0;
    }
    return row;
  });
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4 text-black shadow-sm">
      <h3 className="mb-4 text-sm font-semibold text-black">Test Accuracy (%)</h3>
